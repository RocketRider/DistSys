...
http_status_entry_t http_status_list[] = {
    ...
    { HTTP_STATUS_MOVED_PERMANENTLY, 			"Moved Permanently"               , HTML_301	},
    { HTTP_STATUS_NOT_MODIFIED, 				"Not Modified"                    , ""			},
    { HTTP_STATUS_BAD_REQUEST, 					"Bad Request"                     , HTML_400	},
    { HTTP_STATUS_FORBIDDEN, 					"Forbidden"                       , HTML_403	},
    { HTTP_STATUS_NOT_FOUND, 					"Not Found"                       , HTML_404	},
    { HTTP_STATUS_RANGE_NOT_SATISFIABLE, 		"Requested Range Not Satisfiable" , ""			},
    { HTTP_STATUS_INTERNAL_SERVER_ERROR, 		"Internal Server Error"           , HTML_500	},
    { HTTP_STATUS_NOT_IMPLEMENTED,		 		"Not Implemented"                 , HTML_501	}
};

/*
 * Name: ishex
 * Zweck: Überprüft ob es sich um ein asci zeichen eines hex strings handelt
 * In-Parameter: int x (Das zu überprüfende Zeichen)
 * Out-Parameter: -
 * Globale Variablen: -
 * Rückgabewert: int (Wahrheitswert ob es ein 'hex Zeichen' ist)
 * Quelle: http://rosettacode.org/wiki/URL_decoding#C
 */
...
/*
 * Name: http_decode_url
 * Zweck: Dekodiert eine URL nach RFC standard
 * In-Parameter: const char *s (Die noch enkodierte url)
 * Out-Parameter: char* dec (Ausgabe der enkodierten url)
 * Globale Variablen: -
 * Rückgabewert: int (länge des ausgabe strings)
 * Quelle: http://rosettacode.org/wiki/URL_decoding#C
 */
 ...
 * In/Out-Parameter: http_request_t *http_request (Http Request)
 * Globale Variablen: -
...
char* http_create_header(http_request_t *http_request) {
	...
	int http_status_list_size = sizeof(http_status_list)
			/ sizeof(http_status_list[0]);
	...
	sprintf(response + strlen(response), "%d %s\n",
			http_request->response_status,
			http_status_list[http_status_list_index].text);

	//Date
	struct tm *my_tm = gmtime(&(http_request->request_time));
	strftime(response + strlen(response),
			HTTP_MAX_HEADERSIZE - strlen(response),
	...
	if (http_request->response_status == HTTP_STATUS_PARTIAL_CONTENT) {
		...
		sprintf(response + strlen(response), "Content-Range: bytes %u-%u/%d\n",
				http_request->range_begin, http_request->range_end,
	...
	if (http_request->location != NULL && strlen(http_request->location) > 0) {
		if (strlen(http_request->location)
				> (HTTP_MAX_HEADERSIZE - strlen(response) - 11 - 19 - 1)) {
...
int http_parse_header(http_request_t *http_request) {
	...
	for (int i = 0; i < HTTP_METHOD_LIST_SIZE - 1; i++) {
		if (memcmp(http_request->request_buffer, http_method_list[i].name,
				strlen(http_method_list[i].name)) == 0) {
	...
	if (http_request->url != NULL) {
		...
		if (http_request->host != NULL && url_size >= host_size) {
			if (memcmp(http_request->url, http_request->host, host_size) == 0) {
				memmove(http_request->url, http_request->url + host_size,
						url_size - host_size + 1);
			}
		}
	} else {
		err_print("Can't allocate memory!");
	...
	if (modified_since != NULL) {
		...
		if (strptime(modified_since, "%a, %d %b %Y %H:%M:%S GMT",
				&my_tm) != NULL) {
 ...
 * In/Out-Parameter: http_request_t *http_request (Http Request Struktur)
 * Globale Variablen: -
 * Rückgabewert: -	
 */
