...
int static server_answer_error(http_request_t *request) {
	...
	for (http_status_list_index = 0;
		...
		if (http_status_list[http_status_list_index].code == request->response_status) {
	...
	if (request->content_length > 0) {
		ret = write_to_socket(request->sd, html, request->content_length, SERV_WRITE_TIMEOUT);//Connection is closed
		if (ret < 0) {
...
int static server_answer(http_request_t *request) {
	...
	//Check file
	struct stat sb;
	request->file_name = malloc( strlen(request->root_dir) + strlen(request->url) + 1);
	...
	memcpy(request->file_name, request->root_dir, strlen(request->root_dir) + 1);
	...
	if (S_ISDIR(sb.st_mode)) {
		if (*(request->url+strlen(request->url)-1) != '/')
		{
			...
			{
				request->location = calloc(strlen(request->host) + strlen(request->url) + 7 + 11, 1);//2
	...
	//Get mime type
	request->content_type = get_http_content_type_str(get_http_content_type(request->file_name));
	...
	if (request->is_range_request == 1) //Is Range request
	{
		...
		if (request->range_end > request->content_length || request->range_begin >= request->content_length)
		{
	...
	if (request->content_length > 0 && request->method != HTTP_METHOD_HEAD) {
		do{
			...
			if (read_bytes > 0)
			{
				ret = write_to_socket(request->sd, buffer, read_bytes, SERV_WRITE_TIMEOUT);
				if (ret < 0) {
